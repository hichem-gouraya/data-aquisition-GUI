...
# ── Serial port setup ──────────────────────────────────────────────────────────
# Single STM32 serial connection — sends packets: 0x55 | Tmp(f) | Cou(f) | Ppm(f) | Hum(f)
try:
    _ser = serial.Serial(
        port='COM5',
        baudrate=115200,
...
class App(ctk.CTk):
            ...
            "production": {
                ...
                "lines": {
                    ...
                    "courant": {
                        ...
                        "unit": "A",
                        "y_range": (0, 100), "y_label": "Current (A)",
                        "x_label": "Time (ms)", "gauge_max": 100, "rand": (0, 100),
                    },
                },
    ...
    def _build_main_page(self) -> None:
        ...
        gauge_cfgs = [
            ...
            ("courant",     "Current",     "A",    _D["info"],    100,   "production"),
    ...
    def _build_table(self, parent: Any, p: dict) -> None:
        ...
        col_hd = {"time": "TIME", "temperature": "TEMPERATURE (°C)",
                  "courant": "CURRENT (A)", "humidity": "HUMIDITY (%RH)",
                  "H2 ppm": "H₂ CONC. (ppm)"}
    ...
    def _read_packet(self) -> dict | None:
        """
        Read one full data packet from the STM32.
        Protocol (same as FirstSerial.py):
            1 byte  : 0x55  (start flag)
            4 bytes : Tmp   (float32, little-endian)
            4 bytes : Cou   (float32, little-endian)
            4 bytes : Ppm   (float32, little-endian)
            4 bytes : Hum   (float32, little-endian)
        Returns dict with keys temperature, courant, H2 ppm, humidity,
        or None if no complete packet is available yet.
        """
        if _ser is None:
            return None
        try:
            # Hunt for the 0x55 start flag
            if _ser.in_waiting < 1:
                return None
            flag = _ser.read(1)
            if flag != b'\x55':
                # Discard stale bytes until we find the flag on the next call
                return None

            # We have the flag — now read 16 bytes (4 floats × 4 bytes)
            payload = _ser.read(16)
            if len(payload) < 16:
                print("Incomplete packet, skipping...")
                return None

            tmp, cou, ppm, hum = struct.unpack('<ffff', payload)
            return {
                "temperature": tmp,
                "courant":     cou,
                "H2 ppm":      ppm,
    ...
    def _update_loop(self) -> None:
        ...
        try:
            ...
            # Try to get a fresh packet; fall back to last known EMA values
            packet = self._read_packet()

            for tab_key, cfg in self._cfg.items():
                values = {}
                for lk in cfg["lines"]:
                    if packet is not None and lk in packet:
                        values[lk] = packet[lk]
                    else:
                        # No new packet — hold last smoothed value
                        values[lk] = self._ema.get(lk, 0.0)
