...
try:
    ...
    print(f"Serial (current) not connected: {e}")
    _ser = None

# MQ-8 H₂ sensor — change COM port below if on a different port than current sensor
# If your STM32 sends BOTH current AND MQ-8 ADC on the same port, set MQ8_PORT = 'COM5'
# and the app will open a separate handle — change to 'COM6' etc. if separate.
MQ8_PORT = 'COM5'

try:
    _ser_h2 = serial.Serial(
        port=MQ8_PORT,
        baudrate=115200,
        bytesize=serial.EIGHTBITS,
        parity=serial.PARITY_NONE,
        stopbits=serial.STOPBITS_ONE,
        timeout=0.05
    ) if MQ8_PORT != 'COM5' else _ser   # reuse same handle if same port
except Exception as e:
    print(f"Serial (H2) not connected: {e}")
    _ser_h2 = _ser  # fall back to same handle

# ── MQ-8 H₂ calibration constants ─────────────────────────────────────────────
_MQ8_RS_CLEAN_AIR    = 39483.9   # RS measured in clean air (Ω)
_MQ8_RATIO_CLEAN_AIR = 8.0       # RS/R0 ratio in clean air (from MQ-8 datasheet)
_MQ8_R0              = _MQ8_RS_CLEAN_AIR / _MQ8_RATIO_CLEAN_AIR   # ≈ 4935 Ω
_MQ8_RL              = 4700      # Load resistor (Ω)
_MQ8_VCC             = 5.0       # Sensor supply voltage (V)
...
class App(ctk.CTk):
    ...
    def __init__(self) -> None:
        ...
        self._last_current_ma: float = 0.0

        # ── MQ-8 state ──────────────────────────────────────────────────────
        self._last_h2_ppm: float = 0.0   # holds last valid H2 reading
    ...
    def _build_statusbar(self) -> None:
        ...
        h2_status     = "H₂ OK" if _ser_h2 else "H₂ NO SERIAL"
        serial_status = "COM5 CONNECTED" if _ser else "NO SERIAL"
        self._status_label = ctk.CTkLabel(
            bar, text=f"● LIVE  |  Sampling: 100 ms  |  {serial_status}  |  {h2_status}",
    ...
    def _read_current(self) -> float | None:
        """Read current sample from STM32. Returns amps or None."""
    ...
    def _read_h2_ppm(self) -> float | None:
        """
        Read MQ-8 H₂ PPM from STM32 ADC over serial.
        Returns ppm value or None if no new data.
        """
        if _ser_h2 is None or _ser_h2.in_waiting < 4:
            return None
        try:
            raw_bytes = _ser_h2.read(4)
            adc       = int.from_bytes(raw_bytes, byteorder='little')

            # ADC → voltage at STM32 pin (after voltage divider ÷2)
            voltage = (adc / 4095) * 3.3   # STM32 pin voltage
            voltage = voltage * 2           # recover actual sensor output voltage

            # Protect against division by zero (sensor not powered / bad read)
            if voltage <= 0 or voltage >= _MQ8_VCC:
                return self._last_h2_ppm   # return last known good value

            # RS from voltage divider formula
            rs    = ((_MQ8_VCC - voltage) / voltage) * _MQ8_RL
            ratio = rs / _MQ8_R0           # RS/R0

            # PPM from MQ-8 sensitivity curve approximation
            ppm   = 1000.0 * (ratio ** -1.5)
            ppm   = max(0.0, ppm)           # clamp negative values

            self._last_h2_ppm = ppm
            return ppm
        except Exception:
            return None
        ...
        try:
            ...
            for tab_key, cfg in self._cfg.items():
                ...
                for lk, lcfg in cfg["lines"].items():
                    ...
                    if lk == "courant":
                        # ── Real current from STM32 ──────────────────────
                        real = self._read_current()
                        values[lk] = int(real * 1000) if real is not None \
                                     else int(self._ema.get(lk, 0))

                    elif lk == "H2 ppm":
                        # ── Real H₂ PPM from MQ-8 via STM32 ADC ─────────
                        ppm = self._read_h2_ppm()
                        values[lk] = int(ppm) if ppm is not None \
                                     else int(self._last_h2_ppm)
